...
<style>
    .comprehensive-payment-complete {
...
`;
        ...
        // FIXED: Hide promo selection info for regular packages
        function hidePromoSelectionInfo() {
            $('#promoSelectionInfo').addClass('hidden').hide();
            $('#afterPromoRow').hide(); // FIXED: Also hide the "After Promo Deduction" row
            $('#promoDiscount').hide();
            $('#promoInfo').hide();

            console.log(`[2025-08-05 14:22:08] Scraper001: Promo selection info hidden`);
        }
        ...
        const populateDemoSelect = (preserveSelection = true) => {
            ...
            const currentTimestamp = '2025-08-05 14:15:22';
            const currentUser = 'Scraper001';
            ...
            const demoDetails = <?= json_encode($demo_details ?? []) ?>;
            const originalDemoFee = calculateDemoFeeJS();

            // UPDATED: Filter available demos - ONLY show unpaid and partially paid demos
            const availableDemos = allDemos.filter(demo => {
                const demoInfo = demoDetails[demo.value] || {};
                const paidAmount = parseFloat(demoInfo.paid_amount || 0);

                // FIXED: Only show demos that are NOT fully paid
                const isFullyPaid = paidAmount >= (originalDemoFee - 0.01); // Small tolerance for rounding
                return !isFullyPaid; // Hide fully paid demos
            });

            console.log(`[${currentTimestamp}] Demo Filter - User: ${currentUser}
        Total Demos: ${allDemos.length}
        Available (unpaid/partial): ${availableDemos.length}
        Fully Paid: ${allDemos.length - availableDemos.length}`);

            // Add only unpaid/partially paid demos to select
            ...
            // Handle demo payment option visibility
            ...
            if (availableDemos.length === 0) {
                // UPDATED: Hide demo payment completely and show "Payment Complete"
                $demoPaymentOption.addClass('payment-hidden');
                $demoPaymentInput.prop('disabled', true);

                // Add payment complete message
                if ($('#paymentCompleteMessage').length === 0) {
                    $demoPaymentOption.after(`
                <div id="paymentCompleteMessage" class="payment-complete-display">
                    <i class="fa-solid fa-check-circle text-white mr-2"></i>
                    <span class="text-white font-semibold">Payment Complete</span>
                    <br><small class="text-gray-600">All demo sessions have been paid</small>
                </div>
            `);
                }

                console.log(`[${currentTimestamp}] ${currentUser}: All demos completed - showing "Payment Complete"`);
            } else {
                // Remove payment complete message if it exists
                $('#paymentCompleteMessage').remove();
                ...
                console.log(`[${currentTimestamp}] ${currentUser}: Demos available - ${statusText}`);
            }

            // Rest of the existing code for change handlers...
        ...
        // FIXED: Calculate total with proper regular package handling
        const calculateTotal = (program, package) => {
            let subtotal = parseFloat(program.total_tuition || 0);
            let discount = 0;

            // FIXED: Only apply discount if there's actually a promo package selected
            if (package && package.package_name && package.package_name !== 'Regular Package' && package.package_name !== 'Regular') {
                ...
                console.log(`[2025-08-05 14:22:08] Scraper001: Promo Package Applied
            Package: ${package.package_name}
            Selection Type: ${selectionType}
            Discount: ₱${discount.toFixed(2)}`);
            } else {
                // FIXED: Regular package - no discount at all
                discount = 0;
                console.log(`[2025-08-05 14:22:08] Scraper001: Regular Package - No Discount Applied`);
            }
            ...
            // FIXED: Only show "After Promo Deduction" if there's actually a discount
            if (discount > 0) {
                $('#afterPromoRow').show();
                $('#afterPromoAmount').text(`₱${finalTotal.toLocaleString()}`);
                console.log(`[2025-08-05 14:22:08] Scraper001: Showing promo deduction - Final: ₱${finalTotal.toFixed(2)}`);
            } else {
                $('#afterPromoRow').hide();
                console.log(`[2025-08-05 14:22:08] Scraper001: Hiding promo deduction - Regular pricing`);
            }
            ...
            // FIXED: Only show promo info if there's actually a discount
            if (discount > 0) {
        ...
        // FIXED: Payment calculation with correct balance for full payment
        // FIXED: Payment calculation that allows manual input override
            ...
            switch (paymentType) {
                case 'full_payment':
                    ...
                    if (remainingBalanceElement && remainingBalanceElement.textContent) {
                        ...
                        amount = parseFloat(balanceText) || 0;
                    } else {
                        const totalRequired = parseFloat($('#finalTotalHidden').val()) || parseFloat(currentProgram.total_tuition);
                ...
                case 'demo_payment':
                    if (demoType) {
                        ...
                        if (selectedOption.length) {
                            amount = parseFloat(selectedOption.data('remaining')) || calculateDemoFeeJS();
                        } else {
        ...
        // FIXED: Enhanced excess demo payment handling with proper allocation
        function validateDemoPayment() {
            ...
            if (paymentType === 'demo_payment' && demoType) {
                ...
                // Only validate that payment is not less than required amount
                if (totalPayment < remainingAmount - 0.01) { // Allow small rounding differences
                    Swal.fire({
                        icon: 'warning',
                        title: 'Invalid Demo Payment Amount',
                        html: `The demo payment amount must be at least ₱${remainingAmount.toFixed(2)}.<br>Please adjust the amount.`,
                        confirmButtonText: 'OK'
                    });
                    return false;
                }
        ...
        async function handleDemoExcessPayment(paymentAmount, requiredAmount, demoType) {
            ...
            // If no excess or very small amount, just process normally
            if (excess <= 0.01) return null;

            // Get the next demo in sequence
            const demoIndex = parseInt(demoType.replace('demo', ''));
            const nextDemoIndex = demoIndex + 1;
            const nextDemoType = nextDemoIndex <= 4 ? `demo${nextDemoIndex}` : null;

            // For the last demo (demo4), show options for excess
            if (!nextDemoType) {
                return await showExcessOptions(excess, paymentAmount, requiredAmount, demoType, null);
            }

            // For demos 1-3, show allocation options including next demo
            return await showExcessOptions(excess, paymentAmount, requiredAmount, demoType, nextDemoType);
        }

        // FIXED: Show consistent options for excess payments
        // FIXED: Only show Allocate and Return options (no credit to account)
        async function showExcessOptions(excessAmount, paymentAmount, requiredAmount, demoType, nextDemoType) {
            return new Promise((resolve) => {
                let modalTitle, modalContent;

                if (nextDemoType) {
                    modalTitle = 'Excess Payment Detected';
                    modalContent = `
                <div style="text-align: left; padding: 10px;">
                    <div style="background: #fff3cd; border: 1px solid #ffc107; padding: 15px; border-radius: 8px; margin-bottom: 20px;">
                        <h4 style="margin: 0 0 10px 0; color: #856404;">
                            <i class="fa-solid fa-exclamation-triangle"></i> Excess Payment
                        </h4>
                        <div style="font-size: 14px; line-height: 1.5;">
                            <strong>Current Demo:</strong> ${demoType.replace('demo', 'Demo ')}<br>
                    ...
                    <div style="display: flex; flex-direction: column; gap: 12px;">
                        <button class="excess-option-btn" data-option="allocate_to_next_demo" style="padding: 15px; border: 2px solid #28a745; background: #d4edda; border-radius: 8px; cursor: pointer; text-align: left;">
                            <div style="font-weight: bold; color: #155724; margin-bottom: 5px;">
                                <i class="fa-solid fa-arrow-right mr-1"></i> Apply to ${nextDemoType.replace('demo', 'Demo ')}
                            </div>
                            <small style="color: #155724;">Apply the excess ₱${excessAmount.toLocaleString()} to ${nextDemoType.replace('demo', 'Demo ')}</small>
                        </button>
                        
                        <button class="excess-option-btn" data-option="return_as_change" style="padding: 15px; border: 2px solid #ffc107; background: #fff3cd; border-radius: 8px; cursor: pointer; text-align: left;">
                            <div style="font-weight: bold; color: #856404; margin-bottom: 5px;">
                                <i class="fa-solid fa-hand-holding-usd mr-1"></i> Return as Change
                            </div>
                            <small style="color: #856404;">Return the excess ₱${excessAmount.toLocaleString()} as cash change.</small>
                        </button>
                    </div>
                </div>
            `;
                } else {
                    // Last demo or no next demo available - only return as change
                    modalTitle = 'Excess Payment for Last Demo';
                    modalContent = `
                <div style="text-align: left; padding: 10px;">
                    <div style="background: #fff3cd; border: 1px solid #ffc107; padding: 15px; border-radius: 8px; margin-bottom: 20px;">
                        <h4 style="margin: 0 0 10px 0; color: #856404;">
                            <i class="fa-solid fa-exclamation-triangle"></i> Excess Payment for Final Demo
                        </h4>
                        <div style="font-size: 14px; line-height: 1.5;">
                            <strong>Current Demo:</strong> ${demoType.replace('demo', 'Demo ')}<br>
                            <strong>Required Amount:</strong> ₱${requiredAmount.toLocaleString()}<br>
                    ...
                    <p style="margin-bottom: 20px;">
                        This is the final demo. The excess will be returned as change.
                    </p>
                        ...
                        document.querySelectorAll('.excess-option-btn').forEach(btn => {
                            ...
                            btn.addEventListener('click', () => {
                                ...
                                const result = {
                                    ...
                                    requiredAmount: requiredAmount,
                                    demoType: demoType,
                                    nextDemoType: nextDemoType,
                                    description: getExcessDescription(option, excessAmount, demoType, nextDemoType),
                                    ...
                                    allocationDetails: {
                                        ...
                                        requiredAmount: requiredAmount,
                                        excessAmount: excessAmount,
                                        processingTime: '2025-08-10 08:13:02'
                                    }
                                };
        ...
        // Helper function for excess description text
        function getExcessDescription(option, excessAmount, demoType, nextDemoType) {
            switch (option) {
                case 'allocate_to_next_demo':
                    return `Excess ₱${excessAmount.toFixed(2)} allocated to ${(nextDemoType || 'next demo').replace('demo', 'Demo ')}`;
                case 'return_as_change':
                    return `Excess ₱${excessAmount.toFixed(2)} returned as change`;
                default:
                    return `Excess payment of ₱${excessAmount.toFixed(2)} processed`;
            }
        }
    ...
    `;
        ...
        const validatePaymentTypes = () => {
            // Hide already paid payment types
            ...
            if (currentBalance <= 0.01) {
                ...
                if (!hasAvailableDemos) {
                    if ($('#comprehensivePaymentComplete').length === 0) {
                        $('.payment-type-option').last().after(`
                    <div id="comprehensivePaymentComplete" class="comprehensive-payment-complete">
                        <div class="complete-icon">
                            <i class="fa-solid fa-medal text-yellow-500 mr-2"></i>
                        </div>
                        <div class="complete-content">
                            <div class="complete-title">🎉 Enrollment Complete!</div>
                            <div class="complete-subtitle">All payments have been successfully processed</div>
                            <div class="complete-details">
                                <small>✅ All demos paid ✅ Full balance settled</small>
                            </div>
                        </div>
                    </div>
                `);
            ...
            if (paidPaymentTypes.includes('full_payment')) {
                $fullPaymentOption.addClass('payment-hidden');
                $fullPaymentInput.prop('disabled', true);
                $fullPaymentStatus.text('(Already paid)').show();
            }
        };
        // =============================================================================================
        ...
        // FIXED: Package selection handler with proper regular package handling
        $('#packageSelect').change(function () {
            ...
            if (!packageName) {
                ...
                // FIXED: Hide promo selection info
                hidePromoSelectionInfo();
                if (currentProgram) {
            ...
            if (packageName === 'Regular Package' || packageName === 'Regular') {
                // FIXED: Properly handle regular package
                currentPackage = null;
                $('#packageInfo').text(`Package: Regular Package`);
                $('#packageDetailsHidden').val('{}');

                // FIXED: Hide promo selection info for regular package
                hidePromoSelectionInfo();

                console.log(`[2025-08-05 14:22:08] Scraper001: Regular Package Selected - No Promo Applied`);

                if (currentProgram) {
                    calculateTotal(currentProgram, null); // Pass null for regular package
                    updateProgram(currentProgram);
                }
                return;
            }

            // Get package details for promo packages
            $.ajax({
                ...
                success: function (packageData) {
                    ...
                    // Show promo selection info for promo packages
                    displayPromoSelectionInfo(packageData);

                    console.log(`[2025-08-05 14:22:08] Scraper001: Promo Package Selected
                Package: ${packageName}
                Selection Type: ${packageData.selection_type}
                Percentage: ${packageData.percentage}%`);
                    ...
                    if (currentProgram) {
                        ...
                        // Show package info for custom payment
        ...
        // FIXED: Payment type change handler that properly updates amounts
        // Enhanced payment type change handler with demo validation
        $('input[name="type_of_payment"]').change(function () {
            ...
            // Remove any existing demo warnings
            $('#demoSelectionWarning').remove();

            if (paymentType === 'demo_payment') {
                $('#demoSelection').show();
                const hasAvailableDemos = populateDemoSelect(true);
                ...
                // Show instruction message for demo selection
                if ($('#demoInstructionMsg').length === 0) {
                    $('#demoSelection').after(`
            ...
            // Clear and update payment amounts
            $('#totalPayment').val('').data('user-modified', false);
            $('#cash').val('');
            $('#change').val('0.00');
        ...
        $('#demoSelect').change(function () {
            ...
            if (paymentType === 'demo_payment') {
                if (!selectedDemo || selectedDemo === '') {
                    // Clear payment fields if no demo selected
                    $('#totalPayment').val('');
                    $('#cashToPay').val('');
                    $('#cash').val('');
                    $('#change').val('0.00');
                    ...
                    // Show warning message
                    if ($('#demoSelectionWarning').length === 0) {
                        $('#demoSelection').after(`
                    <div id="demoSelectionWarning" class="bg-yellow-100 border border-yellow-400 text-yellow-700 px-3 py-2 rounded mt-2 text-sm">
                        <i class="fa-solid fa-exclamation-triangle mr-2"></i>
                        <strong>Warning:</strong> Please select a demo to proceed with demo payment.
                    </div>
                `);
                    }
        ...
        function validateDemoSelection() {
            const paymentType = $('input[name="type_of_payment"]:checked').val();
            const demoType = $('#demoSelect').val();

            if (paymentType === 'demo_payment') {
                if (!demoType || demoType === '') {
                    Swal.fire({
                        icon: 'error',
                        title: 'Demo Selection Required',
                        html: `
                    <div style="text-align: left;">
                        <p><strong>Error:</strong> You must select a specific demo before processing demo payment.</p>
                        <p><strong>Available options:</strong></p>
                        <ul style="margin-left: 20px;">
                            <li>1st Practical Demo</li>
                            <li>2nd Practical Demo</li>
                            <li>3rd Practical Demo</li>
                            <li>4th Practical Demo</li>
                        </ul>
                        <p><strong>Note:</strong> Only unpaid or partially paid demos will appear in the dropdown.</p>
                    </div>
                `,
                        confirmButtonText: 'Select Demo',
                        allowOutsideClick: false
                    }).then(() => {
                        // Focus on the demo select dropdown
                        $('#demoSelect').focus();
                    });
                    return false;
                }

                // Additional validation: Check if selected demo is valid
                const selectedOption = $('#demoSelect').find(`option[value="${demoType}"]`);
                if (selectedOption.length === 0) {
                    Swal.fire({
                        icon: 'error',
                        title: 'Invalid Demo Selection',
                        text: 'The selected demo is not available. Please choose a valid demo from the dropdown.',
                        confirmButtonText: 'OK'
                    });
                    return false;
                }

                console.log(`[2025-08-05 15:32:30] Scraper001: Demo validation passed - Selected: ${demoType}`);
            }
        ...
        $('#posForm').submit(async function (e) {
            ...
            switch (paymentType) {
                ...
                case 'initial_payment':
                    // Calculate required initial payment
                    requiredAmount = currentPackage && currentPackage.selection_type > 2 && currentPackage.custom_initial_payment
                        ? parseFloat(currentPackage.custom_initial_payment)
                        : parseFloat(currentProgram?.initial_fee || 0);

                    if (totalPayment > requiredAmount && requiredAmount > 0) {
                        // Use existing excess payment handler for initial payments
                        excessData = await showExcessPaymentModal(totalPayment, requiredAmount, totalPayment - requiredAmount, paymentType);
                        if (!excessData) {
                            return false; // User cancelled
                        }
                    }
    ...
    `);
            ...
            // Open cash drawer if applicable and connected
            if (paymentType !== 'reservation' && paymentType !== 'initial_payment' && cashDrawerConnected) {
                try {
                ...
                if (response.success) {
                    ...
                    if (excessData) {
                        ...
                        if (response.transaction_id) {
                            successMessage += `<div class="mt-2 text-gray-600">
                        <small>Transaction ID: ${response.transaction_id}</small>
                    </div>`;
                        }
                    }
...
`);
    ...
    setTimeout(() => {
        ...
        // Manual check if full payment was incorrectly hidden
        const hasBalance = window.checkFullPaymentAvailability();
        if (hasBalance) {
            console.log(`[2025-08-05 14:28:20] Scraper001: Full payment availability confirmed`);
        }
    }, 1000);
</script>
...
<script>
    // FIXED: Add this function to manually check balance and restore full payment if needed
    window.checkFullPaymentAvailability = function () {
        const remainingBalanceElement = document.getElementById('remainingBalance');
        const totalAmountElement = document.getElementById('totalAmountDisplay');
        const creditBalElement = document.getElementById('credit_Bal');

        if (!remainingBalanceElement || !totalAmountElement || !creditBalElement) {
            console.error('[2025-08-05 14:28:20] Scraper001: Required balance elements not found');
            return;
        }

        const remainingBalance = parseFloat(remainingBalanceElement.textContent.replace(/[₱,\s]/g, '')) || 0;
        const totalAmount = parseFloat(totalAmountElement.textContent.replace(/[₱,\s]/g, '')) || 0;
        const totalPaid = parseFloat(creditBalElement.textContent.replace(/[₱,\s]/g, '')) || 0;

        console.log(`[2025-08-05 14:28:20] Scraper001: Manual Balance Check
        Remaining Balance Display: ₱${remainingBalance.toFixed(2)}
        Total Amount: ₱${totalAmount.toFixed(2)}
        Total Paid: ₱${totalPaid.toFixed(2)}
        Calculated Balance: ₱${(totalAmount - totalPaid).toFixed(2)}`);

        const $fullPaymentOption = $('label[data-payment-type="full_payment"]');
        const $fullPaymentInput = $('input[name="type_of_payment"][value="full_payment"]');
        const $fullPaymentStatus = $fullPaymentOption.find('.payment-status');

        // Force show full payment if there's a balance
        if (remainingBalance > 0.01 || (totalAmount - totalPaid) > 0.01) {
            $fullPaymentOption.removeClass('payment-hidden');
            $fullPaymentInput.prop('disabled', false);
            $fullPaymentStatus.text(`(Balance: ₱${Math.max(remainingBalance, totalAmount - totalPaid).toFixed(2)})`).show();

            console.log(`[2025-08-05 14:28:20] Scraper001: Full payment restored - balance exists`);

            return true;
        } else {
            console.log(`[2025-08-05 14:28:20] Scraper001: Full payment correctly hidden - no balance`);
            return false;
        }
    };
</script>
