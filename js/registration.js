...
document.addEventListener("DOMContentLoaded", function () {
  ...
  sameAsAboveCheckbox.addEventListener("change", function () {
    if (this.checked) {
      ...
      document.getElementById("emergencyBrgy").value =
        document.getElementById("barangay").value;
      document.getElementById("emergencyPurok").value =
        document.getElementById("purok").value;

      // Copy the hidden field values as well
      document.getElementById("emergency-region-name").value =
        document.getElementById("current-region-name").value;
      document.getElementById("emergency-province-name").value =
        document.getElementById("current-province-name").value;
      document.getElementById("emergency-city-name").value =
        document.getElementById("current-city-name").value;
      document.getElementById("emergency-barangay-name").value =
        document.getElementById("current-barangay-name").value;
  ...
  function validateForm() {
    ...
    if (
      referralOther &&
      referralOther.checked &&
      otherReferral.value.trim() === ""
    ) {
      isValid = false;
  ...
  function addErrorTooltip(element, message) {
    ...
    tooltip.className =
      "error-tooltip absolute bg-red-500 text-white text-sm rounded py-1 px-2 z-10";
  ...
  function submitForm(form) {
    ...
    const requirementCheckboxes = form.querySelectorAll(
    ...
    fetch("functions/registration_process.php", {
      ...
      .then((data) => {
        ...
        if (data.success || data.status === "success") {
          ...
          if (
            data.has_missing_requirements === true &&
              ...
              Swal.fire({
                            ...
                            <div style="text-align: left;">
                                ...
                                <div style="background-color: #d4edda; border: 1px solid #c3e6cb; border-radius: 5px; padding: 15px; margin: 10px 0;">
                                    ...
                                    <ul style="color: #155724; margin: 5px 0 0 0; padding-left: 20px;">
                                        ${
                                          data.submitted_requirements &&
                                          data.submitted_requirements.length > 0
                                            ? data.submitted_requirements
                                                .map((req) => `<li>${req}</li>`)
                                                .join("")
                                            : "<li>None</li>"
                ...
                if (result.isConfirmed) {
                  ...
                  window.location.href =
                    data.redirect || "student_management.php";
                }
              });
            } else {
            ...
            if (typeof Swal !== "undefined") {
              Swal.fire({
                ...
                text:
                  data.message ||
                  "All requirements have been submitted! Registration is now complete.",
                allowOutsideClick: false,
              }).then(() => {
                window.location.href =
                  data.redirect || "student_management.php";
              ...
              alert(
                "Registration Complete: " +
                  (data.message || "All requirements have been submitted!")
              );
      ...
      .catch((error) => {
        ...
        if (typeof Swal !== "undefined") {
          ...
          alert(
            "Submission Error: There was a problem connecting to the server. Please try again later."
          );
        }
      });
  }
    ...
    const requiredDocs = [
      {
        ...
        checkbox: 'input[name="requirements[]"][value="PSA Birth Certificate"]',
      },
      {
        name: "TOR, ALS Certificate or FORM 137",
        checkbox:
          'input[name="requirements[]"][value="TOR, ALS Certificate or FORM 137"]',
      },
      {
        name: "Passport size photos",
        checkbox: 'input[name="requirements[]"][value="Passport size photos"]',
      },
    ];
  ...
  function setupRequirementsValidation() {
    const requirementCheckboxes = document.querySelectorAll(
  ...
  allInputs.forEach((input) => {
    input.addEventListener("input", function () {
      ...
      if (
        (this.name === "user_contact" || this.name === "emergencyContact") &&
        this.value.trim() !== ""
      ) {
        if (!validatePhoneNumber(this.value)) {
          ...
          addFieldErrorMessage(
            this,
            "Please enter a valid phone number (09XXXXXXXXX or +639XXXXXXXXX)"
          );
        }
      }
    });
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .getElementById("editReferralOther")
    .addEventListener("change", function () {
      document.getElementById("editOtherReferralDiv").style.display = this
        .checked
        ? "block"
        : "none";
    });
  ...
  document
    .getElementById("editReferralNone")
    .addEventListener("change", function () {
  ...
  document
    .getElementById("editPhotoUpload")
    .addEventListener("change", function (event) {
      ...
      if (file) {
        ...
        reader.onload = function (e) {
          ...
          document
            .getElementById("editPhotoPreview")
        ...
        .then((data) => {
          ...
          document.getElementById("studentDetailsContent").innerHTML = `
                        <div class="mb-4">
                            <img src="functions/${
                              data.photo_path
                            }" alt="Student Photo" class="w-22 h-22 object-cover rounded border border-gray-300" />
                        </div>
                        <p><strong>Name:</strong> ${data.last_name}, ${
            data.first_name
          } ${data.middle_name || ""}</p>
                        ...
                        <p><strong>Address:</strong> ${data.region}, ${
            data.province
          }, ${data.city}, ${data.brgy}</p>
                        <p><strong>Birthplace:</strong> ${data.birth_region}, ${
            data.birth_province
          }, ${data.birth_city}, ${data.birth_brgy}</p>
                        ...
                        <p><strong>Emergency Contact:</strong> ${
                          data.emergency_name
                        } (${data.relationship}) - ${data.emergency_contact}</p>
                        <p><strong>Referal:</strong> ${data.referral} ${
            data.other_referral ? "(" + data.other_referral + ")" : ""
          }</p>
...
function openEditModal(studentData) {
  ...
  document
    .getElementById("editStudentForm")
    .setAttribute("data-student-id", studentData.id);
  ...
  document.getElementById("editBirthProvince").value =
  ...
  document.getElementById("editEducationalAttainment").value =
    studentData.educational_attainment;
  ...
  document.getElementById("editEmergencyEmail").value =
    studentData.emergency_email || ""; // Added missing field
  document.getElementById("editEmergencyContact").value =
    studentData.emergency_contact;
  document.getElementById("editEmergencyRegion").value =
    studentData.emergency_region;
  document.getElementById("editEmergencyProvince").value =
    studentData.emergency_province;
  document.getElementById("editEmergencyCity").value =
    studentData.emergency_city;
  document.getElementById("editEmergencyBrgy").value =
    studentData.emergency_brgy;
  document.getElementById("editEmergencyPurok").value =
    studentData.emergency_purok || "";
  ...
  if (studentData.referral === "none") {
    ...
    document.getElementById("editOtherReferral").value =
      studentData.other_referral || "";
  }

  document.getElementById("editKnowledgeSource").value =
    studentData.knowledge_source;
  ...
  if (studentData.requirements) {
    ...
    const requirements = studentData.requirements
      .split(",")
      .map((item) => item.trim());

    // Check for PSA Birth Certificate
    document.getElementById("editReqBirthCert").checked = requirements.some(
      (req) =>
        req.includes("PSA Birth Certificate") ||
        req.includes("PSA") ||
        req.includes("Birth Certificate")
    );

    // Check for TOR, ALS or FORM 137
    document.getElementById("editReqTOR").checked = requirements.some(
      (req) =>
        req.includes("TOR") || req.includes("ALS") || req.includes("FORM 137")
    );

    // Check for Photos
    document.getElementById("editReqPhotos").checked = requirements.some(
      (req) =>
        req.includes("Passport size photos") ||
        req.includes("Photos") ||
        req.includes("Passport photos")
    );
  } else {
  ...
  if (studentData.photo_path) {
    document.getElementById(
      "editCurrentStudentPhoto"
    ).src = `functions/${studentData.photo_path}`;
    document
      .getElementById("editCurrentPhotoPreview")
      .classList.remove("hidden");
  } else {
...
function processUpdate(form, studentId) {
  ...
  const referralType =
    document.querySelector('input[name="referral"]:checked')?.value || "none";
    ...
    .then((data) => {
      if (data.status === "success") {
        ...
        Swal.fire({
          ...
          text:
            "Error: " +
            (data.message || "Failed to update student information"),
          confirmButtonColor: "#3085d6",
        });
      }
    })
...
function copyAddress() {
  if ($("#sameAsAbove").is(":checked")) {
    ...
    // Disable emergency address fields
    $(
      "#emergencyRegion, #emergencyProvince, #emergencyCity, #emergencyBrgy, #emergencyPurok"
    ).prop("disabled", true);
  } else {
    // Enable emergency address fields
    $(
    ...
    $("#emergencyProvince, #emergencyCity, #emergencyBrgy")
      .empty()
...
function initializeAddressDropdowns(
  ...
  $.getJSON(
    ...
    function (data) {
      ...
      dropdown
        .empty()
        .append('<option value="" selected disabled>Choose Region</option>');
  ...
  $(regionSelector).on("change", function () {
    ...
    $(provinceSelector)
      .empty()
      .append('<option value="" selected disabled>Choose Province</option>');
    $(citySelector)
      .empty()
      .append('<option value="" selected disabled>Choose City</option>');
    $(barangaySelector)
      .empty()
      .append('<option value="" selected disabled>Choose Barangay</option>');

    $.getJSON(
      "../philippine-address-selector-main/ph-json/province.json",
      function (data) {
  ...
  $(provinceSelector).on("change", function () {
    ...
    $.getJSON(
      "../philippine-address-selector-main/ph-json/city.json",
      function (data) {
  ...
  $(citySelector).on("change", function () {
    ...
    $.getJSON(
      "../philippine-address-selector-main/ph-json/barangay.json",
      function (data) {
...
function initializeEditAddressDropdowns(
  addressType,
  regionSelector,
  provinceSelector,
  citySelector,
...
function selectDropdownOption(
  ...
  return new Promise((resolve) => {
    ...
    $.getJSON(jsonUrl, function (data) {
      ...
      if (!option) {
        option = items.find(
          (item) => item[textProp].toLowerCase() === value.toLowerCase()
        );
      }
      ...
      if (option) {
        ...
        if (
          $(selector + ' option[value="' + option[valueProp] + '"]').length ===
          0
        ) {
          $(selector).append(
            $("<option></option>")
...
function openEditModal(studentData) {
  ...
  initializeEditAddressDropdowns(
    "",
    "#editRegion",
    "#editProvince",
    "#editCity",
  ...
  if (!$("#editRegionName").length) {
    ...
    $("#editStudentForm").append(
      '<input type="hidden" id="editBirthRegionName" name="birth_region_name">'
    );
    $("#editStudentForm").append(
      '<input type="hidden" id="editBirthProvinceName" name="birth_province_name">'
    );
    $("#editStudentForm").append(
      '<input type="hidden" id="editBirthCityName" name="birth_city_name">'
    );
    $("#editStudentForm").append(
      '<input type="hidden" id="editBirthBarangayName" name="birth_barangay_name">'
    );

    $("#editStudentForm").append(
      '<input type="hidden" id="editEmergencyRegionName" name="emergency_region_name">'
    );
    $("#editStudentForm").append(
      '<input type="hidden" id="editEmergencyProvinceName" name="emergency_province_name">'
    );
    $("#editStudentForm").append(
      '<input type="hidden" id="editEmergencyCityName" name="emergency_city_name">'
    );
    $("#editStudentForm").append(
      '<input type="hidden" id="editEmergencyBarangayName" name="emergency_barangay_name">'
    );
  }
...
function copyEditAddress() {
  ...
  if (sameAsAbove) {
    ...
    // Disable emergency address fields
    $(
      "#editEmergencyRegion, #editEmergencyProvince, #editEmergencyCity, #editEmergencyBrgy, #editEmergencyPurok"
    ).prop("disabled", true);
  } else {
    // Enable emergency address fields
    $(
    ...
    $("#editEmergencyProvince, #editEmergencyCity, #editEmergencyBrgy")
      .empty()
      .append('<option value="" selected disabled>Choose</option>');
